...
namespace rest
{
    Dispatcher::Dispatcher( const std::string & endpoint ) : _networkInfoProvider{ std::make_unique< NetworkInfoProvider >() }
    {
    ...
    void Dispatcher::initHandlers()
    {
        _listener->support( web::http::methods::GET,
                            std::bind( &Dispatcher::handleMessage, this, web::http::methods::GET, std::placeholders::_1 ) );
        _listener->support( web::http::methods::PUT,
                            std::bind( &Dispatcher::handleMessage, this, web::http::methods::PUT, std::placeholders::_1 ) );
        _listener->support( web::http::methods::POST,
                            std::bind( &Dispatcher::handleMessage, this, web::http::methods::POST, std::placeholders::_1 ) );
        _listener->support( web::http::methods::DEL,
                            std::bind( &Dispatcher::handleMessage, this, web::http::methods::DEL, std::placeholders::_1 ) );
        _listener->support( web::http::methods::PATCH,
                            std::bind( &Dispatcher::handleMessage, this, web::http::methods::PATCH, std::placeholders::_1 ) );
    }

    void Dispatcher::handleMessage( const std::string & method, const web::http::http_request & message )
    {
        std::cout << "Handling: " << method << " on " << message.relative_uri().path() << std::endl;
    }
